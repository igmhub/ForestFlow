...
def plot_arinyo_z(
    ...
    for i in range(len(name_params)):
        ...
        print(name_params[i])
        s_bias = np.mean(np.abs(ari_emu) - np.abs(ari_cen))
        s_pred = np.std(np.abs(ari_emu) - np.abs(ari_cen))
        print("cen-emu", s_bias * 100, s_pred * 100)

        s_bias = np.mean(np.abs(ari_seed) - np.abs(ari_cen))
        s_pred = np.std(np.abs(ari_seed) - np.abs(ari_cen))
        print("cen-seed", s_bias * 100, s_pred * 100)

        ax1.plot(
            z_central, norm * np.abs(ari_cen), ":o", color=col, lw=2, alpha=0.7
        )
        ax1.plot(
            z_central,
    ...
    ax[2].set_ylim(5e-2, 4)
    ax[1].set_ylim(-0.52, 0.52)
    ax[3].set_ylim(-0.52, 0.52)
    ...
    line3 = Line2D(
        [0], [0], label="Seed", color="k", ls="--", marker="s", linewidth=2
    )
