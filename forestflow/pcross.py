...
def kaiser(bias1, bias2, beta_rsd1, beta_rsd2, mu):
    ...
    Returns:
        float: Kaiser factor.
    """
    return bias1*bias2 * (1 + beta_rsd1 * mu**2 + beta_rsd2 * mu**2 + beta_rsd1*beta_rsd2 * mu**4)
...
def Px_Mpc_withSiIII(
    ...
    **P3D_kwargs
    ...
    import hankl
    if bias_alpha is None:
        bias_alpha = P3D_kwargs['bias']
    if beta_alpha is None:
        beta_alpha = P3D_kwargs['beta']
    ...
    if P3D_mode == "cart":
        # assume P3D_Mpc is a function of (z, kpar, kperp)
        P3D_eval_alpha = P3D_Mpc(z=z, kpar=kpar2d, kperp=kperp2d, **P3D_kwargs)
    elif P3D_mode == "pol":
        # assume P3D_Mpc is a function of (z, k, mu)
        P3D_eval_alpha = P3D_Mpc(z=z, k=k2d, mu=mu2d, **P3D_kwargs)
    # get the P3D for SiIII
    kaiser_SiIII = kaiser(bias_SiIII, bias_SiIII, beta_SiIII, beta_SiIII, mu2d)
    # get the linear power at this redshift
    P3D_lin = Arinyo.linP_Mpc(z, k2d)
    P3D_eval_SiIII = kaiser_SiIII * P3D_lin
    # get the cross term
    kaiser_cross = kaiser(bias_alpha, bias_SiIII, beta_alpha, beta_SiIII, mu2d)
    P3D_cross = kaiser_cross * P3D_lin
    
    # combine
    P3D_eval = P3D_eval_alpha + P3D_eval_SiIII + P3D_cross
