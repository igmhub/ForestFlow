...
# %%
colors = ['crimson']
fig, ax = plt.subplots(ncols=1, nrows= len(sim_labels),figsize = (10,2*len(sim_labels)), sharey=True, sharex=True)

for c in range(len(sim_labels)):
    ax[c].plot(z_grid, np.nanmedian(fractional_error_P1D[c],1), ls = '--', marker = 'o',markersize=2, color = colors[0])
    ax[c].fill_between(z_grid, np.nanmedian(fractional_error_P1D[c],1) - sigma68(fractional_error_P1D[c].flatten()),np.nanmedian(fractional_error_P1D[c],1) + sigma68(fractional_error_P1D[c].flatten()), color = colors[0], alpha = 0.4)
...
for c in range(len(sim_labels)):
    ...
    for mi in range(int(len(mu_lims))):
        ...
        ax[c].plot(k_masked, frac_err_masked, ls = '--', marker = 'o',markersize=2, color = colors[mi],label=f'${mu_lims[mi][0]}\leq \mu \leq {mu_lims[mi][1]}$')
        ax[c].fill_between(k_masked, frac_err_masked - frac_err_err_masked,frac_err_masked + frac_err_err_masked, color = colors[mi], alpha = 0.4)
